...
public class NeuralNetworkDependencyParser extends AbstractDependencyParser {
	...
	public CoNLLSentence parse(List<Term> termList) {
		...
		List<Integer> heads = new ArrayList<>(termList.size());
		List<String> deprels = new ArrayList<>(termList.size());
		...
		for (int i = 0; i < wordArray.length; ++i) {
			...
			if (index < 0) {
				...
				CoNLLWord.ROOT.FOLLOW.add(wordArray[i]);
				continue;
			}
			wordArray[i].HEAD = wordArray[index];
			wordArray[index].FOLLOW.add(wordArray[i]);
		}
