...
public class Vertex {
	 ...
	 * @param from
	 */
	public void updateFromWithNature(Vertex from) {
		double weight = from.weight + MathTools.calculateWeight(from, from.guessNature(), this, guessNature());
		if (this.from == null || this.weight > weight) {
			this.from = from;
			this.weight = weight;
		}
...
//                break;
	...
	public List<Vertex> splitByNature() {
		List<Vertex> vertexes = new LinkedList<>();
		for (int i = 0; i < attribute.nature.length; i++) {
			vertexes.add(new Vertex(word, realWord, new CoreDictionary.Attribute(attribute.nature[i], attribute.totalFrequency), wordID));
		}
		return vertexes;
	}
