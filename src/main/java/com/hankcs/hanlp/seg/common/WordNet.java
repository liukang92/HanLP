...
//                            listIteratorTo.remove();
	 ...
	 * @return
	 */
	public LinkedList<Vertex>[] getSingleNatureVertexes() {
		LinkedList<Vertex>[] newVertexes = new LinkedList[vertexes.length];
		for (int i = 0; i < vertexes.length; ++i) {
			newVertexes[i] = new LinkedList<>();
			for(Vertex v : vertexes[i]){
				newVertexes[i].addAll(v.splitByNature());
			}
		}
		return newVertexes;
	}
